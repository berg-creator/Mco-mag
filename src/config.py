...
#
# Двенадцать часов — это два поста в сутки. Публикация по расписанию
# запускается чаще, четыре раза в день, и это намеренно: лишний запуск просто
# увидит, что рано, и ничего не сделает, зато пропущенный крон не сдвигает
# ленту. Четыре поста в сутки для нового канала — перебор: читатель не успевает,
# курируемые базы съедаются вчетверо быстрее, а решение по каждому посту
# принимает человек кнопкой.
...
# Какие рубрики уже работают. Рубрика попадает в план только когда её база
# наполнена: пустая база даёт не «поменьше постов этой рубрики», а перекос
# квоты в пользу остальных — и канал снова выглядит одинаковым.
# С 10 сентября 2026 работают все четыре, веса берутся из RUBRICS.
ACTIVE_RUBRICS: tuple[str, ...] = ("history", "fresh", "worn", "unreleased")
